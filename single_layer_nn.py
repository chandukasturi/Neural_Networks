...
class SingleLayerNN(object):
    def __init__(self, input_dimensions=2,number_of_nodes=4):
        ...
        """
        """As bias is included in the input we add increment the size of input matrix by one"""
    ...
    def predict(self, X):
        ...
        actual_value = (np.dot(self.weights, self.X) >= 0).astype(int)
        return actual_value
    ...
    def train(self, X, Y, num_epochs=10,  alpha=0.1):
        ...
        for epoch in range(0,num_epochs):
            ...
            W_old = alpha * np.dot(error, self.X.T)
            self.weights = self.weights + W_old
            actual_value = self.predict(X)
