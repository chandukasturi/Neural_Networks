...
class CNN(object):
    ...
    def set_training_flag(self, layernumber=[], layernames="", trainable_flag=True):
        """
        ...
        :param layernames: a string or a list of strings (if both layer_number and layer_name are specified, layer number takes precedence).
        :param trainable_flag: Set trainable flag
        :return: None
        """
        if layernumber:
            if type(layernumber) is list:
                for layerNumber in layernumber:
            ...
            if type(layernames) is list:
                ...
                self.model.get_layer(name=self.toLowerCase(layernames)).trainable = trainable_flag
